...
function ParticipantAvatar({ participant, index }: { participant: DocumentsPresenceParticipant; index: number }) {
  ...
  return (
    <span
      className="documents-presence-avatar"
...
export function DocumentsPresence({ participants, typingNames }: DocumentsPresenceProps) {
  ...
  return (
    <div className="relative flex shrink-0 items-center gap-2" data-testid="documents-presence">
      {message ? (
        <span
          className="hidden max-w-[220px] truncate text-xs sm:inline"
          style={{ color: 'var(--documents-el-muted)' }}
          data-testid="documents-typing-indicator"
        >
          {message}
        </span>
      ) : null}
      ...
      <div className="flex items-center -space-x-2" aria-label={heading}>
        ...
        {hiddenCount > 0 ? (
          <button
            type="button"
            className="documents-presence-more"
            onClick={() => setOpen((value) => !value)}
            aria-expanded={open}
            aria-label={`View ${hiddenCount} more collaborator${hiddenCount === 1 ? '' : 's'}`}
            data-testid="documents-presence-more"
          >
            +{hiddenCount}
          </button>
        ) : null}
      ...
      {open ? (
        <div
          ...
          data-testid="documents-presence-popover"
        >
          <p className="px-2 pb-2 text-xs font-medium" style={{ color: 'var(--documents-el-muted)' }}>
            {heading}
          </p>
            ...
            {participants.map((participant) => (
              <li key={participantKey(participant)} className="flex items-center gap-2 rounded-lg px-2 py-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs" style={{ color: 'var(--documents-el-muted)' }}>
                    {participant.mode === 'edit' ? 'Editing' : 'Viewing'}
