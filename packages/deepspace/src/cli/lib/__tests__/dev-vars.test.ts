...
import { extractCustomDevVars, parseDevVars } from '../dev-vars'
import { renderSecretsCache, stripGeneratedSecretsCache } from '../secrets'
...
describe('extractCustomDevVars', () => {
  ...
  it('strips generated secrets cache before preserving custom vars for unlinked runs', () => {
    const input = [
      ...
      renderSecretsCache(
        { API_KEY: 'stale' },
        { appId: 'app_01HZXYABCDEFGHJKMNPQRSTVWX', configName: 'prd' },
      ),
    ].join('\n')
