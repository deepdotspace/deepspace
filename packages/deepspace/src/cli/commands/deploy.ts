...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    // this, deploy would canonicalize server bindings while `[vars].APP_NAME`
    // continued to carry the raw name — splitting identity across surfaces.
    // (Scope keys are unaffected: `SCOPE_ID = app:${APP_ID}` keys to the
    // immutable app id, not the name.) Fail-fast so the dev fixes
    // wrangler.toml once and every surface reads from the same source of
    // truth.
    ...
    // Advisory schema lint before the build. At runtime these warnings only
    // reach the worker console; a deploy is the last moment to catch a
    // schema that implies more enforcement than it delivers (e.g. a
    // visibilityField no role actually reads through). Never blocks.
