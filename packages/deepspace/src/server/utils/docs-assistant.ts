...
const indexedChunkCache = new WeakMap<DocsCorpusChunk, IndexedDocsChunk>()

/** Validate the generated assistant index before any content reaches a model. */
export function parseDocsCorpus(value: unknown): DocsCorpusChunk[] {
  if (!Array.isArray(value)) throw new Error('Docs assistant index must be an array')
  if (value.length > MAX_CHUNKS) {
    throw new Error(`Docs assistant index exceeds ${MAX_CHUNKS} chunks`)
  }
  const chunks: DocsCorpusChunk[] = []
  let totalChars = 0
  for (let index = 0; index < value.length; index++) {
    const raw = value[index]
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
      throw new Error(`Docs assistant chunk ${index} must be an object`)
    }
    ...
    if (
      ...
      throw new Error(`Docs assistant chunk ${index} has invalid required fields`)
    }
    if (chunk.text.length > MAX_CHUNK_CHARS) {
      throw new Error(`Docs assistant chunk ${chunk.id} exceeds ${MAX_CHUNK_CHARS} characters`)
    }
    totalChars += chunk.text.length
    if (totalChars > MAX_CORPUS_CHARS) {
      throw new Error(`Docs assistant corpus exceeds ${MAX_CORPUS_CHARS} characters`)
    }
    chunks.push({
      id: chunk.id,
      route: chunk.route,
...
/** Deterministic local lexical retrieval used by the docs agent tools. */
...
export function readDocsRoute(
  corpus: DocsCorpusChunk[],
  route: string,
): Array<Pick<DocsCorpusChunk, 'route' | 'title' | 'heading' | 'text'>> {
  const normalizedRoute = normalizeDocsRoute(route)
  return corpus
...
export function buildDocsAssistantTools(corpus: DocsCorpusChunk[]): ToolSet {
  return {
    docs_search: tool({
      description:
      ...
      execute: async ({ query, limit }) => ({
        results: searchDocsCorpus(corpus, query, limit ?? 6),
      }),
    }),
    docs_read: tool({
      description:
        'Read bounded source chunks for one exact documentation route returned by docs_search.',
      inputSchema: z.object({ route: z.string().trim().startsWith('/').max(500) }),
      execute: async ({ route }) => ({
        route: normalizeDocsRoute(route),
        chunks: readDocsRoute(corpus, route),
      }),
    }),
  }
}

export function buildDocsAssistantPrompt(
  appName: string,
  currentRoute: string | undefined,
): string {
  return [
    `You are the public documentation assistant for "${appName}" on DeepSpace.`,
    'Answer only from the documentation available through docs_search and docs_read.',
    'Search before answering unless the user is only greeting you.',
    'Use docs_read when search excerpts are insufficient or ambiguous.',
    ...
    ...(currentRoute
      ? [`The reader is currently viewing ${normalizeDocsRoute(currentRoute)}.`]
      : []),
  ].join('\n')
}
...
function canonicalSearchTerm(term: string): string {
  ...
  if (['doc', 'docs', 'document', 'documents', 'documentation'].includes(term)) return 'docs'
  if (
...
function indexChunk(chunk: DocsCorpusChunk): IndexedDocsChunk {
...
export function normalizeDocsRoute(value: string): string {
