...
import { markdownUrlForRoute } from './routing'
import type { DocsGraph, DocsNavigationNode, DocsPage } from './types'

export const AGENT_SKILLS_SCHEMA = 'https://schemas.agentskills.io/discovery/0.2.0/schema.json'

export interface DocsSkillArtifacts {
  name: string
  description: string
  markdown: string
...
export function createDocsSkillArtifacts(graph: DocsGraph, baseUrl?: string): DocsSkillArtifacts {
  const name = docsSkillName(graph.config.name)
  const description = docsSkillDescription(graph.config.name, graph.config.description)
  const root = baseUrl?.replace(/\/$/, '') ?? ''
  const markdown = renderDocsSkill(graph, name, description, root)
...
export function docsSkillName(value: string): string {
...
function docsSkillDescription(name: string, description?: string): string {
...
function renderDocsSkill(
  ...
  const lines = [
    ...
    graph.config.description ??
      `Use the published ${graph.config.name} documentation as the source of truth.`,
    '',
    '## Grounding workflow',
    '',
    ...
    '- Preserve security, authentication, billing, and deployment boundaries stated in the docs.',
...
function renderNavigation(
  ...
  const visit = (nodes: DocsNavigationNode[], depth: number): void => {
...
function renderPageLink(page: DocsPage, root: string, markdownRoute: string): string {
  return `- [${page.title}](${root}${markdownRoute})` +
