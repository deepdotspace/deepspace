  ...
  return (
    <article className="docs-article" ref={articleRef}>
      ...
      <header className="docs-article-header">
        ...
        {data.page.description && <p className="docs-lede">{data.page.description}</p>}
      </header>
      {children ?? <div className="docs-prose" dangerouslySetInnerHTML={{ __html: data.page.html }} />}
      ...
      <nav className="docs-pagination" aria-label="Page navigation">
        {data.previous ? (
          <a rel="prev" href={data.previous.route}><small>Previous</small><span><ChevronRightIcon className="is-back" />{data.previous.title}</span></a>
        ) : <span />}
        {data.next ? (
          <a rel="next" href={data.next.route}><small>Next</small><span>{data.next.title}<ChevronRightIcon /></span></a>
        ) : <span />}
      </nav>
    </article>
  )
...
export function ContextRail({
  ...
  return (
    <aside className="docs-context-rail" aria-label="Page context">
      {headings.length > 0 && (
...
function ApiPlayground({ operation }: { operation: NonNullable<DocsRuntimeData['page']['openapi']> }): ReactElement | null {
  ...
  return (
    <section className="docs-playground" data-playground>
      <div className="docs-playground-title"><div><span className={`method method-${operation.method.toLowerCase()}`}>{operation.method}</span><strong>Try this operation</strong></div><code>{url}</code></div>
...
function useArticleEnhancements(
  ...
  useEffect(() => {
    ...
    article.querySelectorAll<HTMLPreElement>('.docs-prose pre').forEach((pre) => {
      if (pre.querySelector('.docs-code-actions')) return
      const controls = document.createElement('div')
      controls.className = 'docs-code-actions'
    ...
    article.querySelectorAll<HTMLElement>('[data-tab-group]').forEach((group, groupIndex) => {
      if (group.querySelector(':scope > .docs-tab-buttons')) return
      const tabs = Array.from(group.querySelectorAll<HTMLElement>(':scope > [data-tab]'))
      if (tabs.length === 0) return
      const controls = document.createElement('div')
      controls.className = 'docs-tab-buttons'
      ...
      const activate = (activeIndex: number, broadcast = false): void => {
        ...
        if (broadcast) {
          ...
          if (label) {
            safeStorageSet('deepspace-docs-tab-selection', label)
            document.dispatchEvent(new CustomEvent('deepspace-docs-tab-change', {
      ...
      tabs.forEach((tab, tabIndex) => {
        ...
        button.type = 'button'
        button.id = `docs-tab-${groupIndex}-${tabIndex}`
      ...
      const title = group.querySelector(':scope > .docs-code-group-title')
      ...
      document.addEventListener('deepspace-docs-tab-change', sync)
      const stored = safeStorageGet('deepspace-docs-tab-selection')
      const initial = stored ? labels.findIndex((label) => label === stored) : -1
      activate(initial >= 0 ? initial : 0)
      cleanups.push(() => { document.removeEventListener('deepspace-docs-tab-change', sync); controls.remove() })
    })
