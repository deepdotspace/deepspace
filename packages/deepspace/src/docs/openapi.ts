...
export interface ParsedOpenApi {
  ...
  route: string
  raw: string
  operations: DocsOpenApiOperation[]
}

export function parseOpenApi(appDir: string, config: DocsOpenApiConfig): ParsedOpenApi {
  const sourcePath = resolve(appDir, config.source)
  assertOpenApiSourcePath(appDir, sourcePath, config.source)
  if (!existsSync(sourcePath)) {
    throw new DocsError(`OpenAPI source not found: ${config.source}`, 'docs_openapi_missing', [
  ...
  const operations: DocsOpenApiOperation[] = []
  ...
  for (const [path, pathItem] of Object.entries(document.paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      } satisfies Omit<DocsOpenApiOperation, 'codeSamples'>
      operations.push({
        ...normalized,
...
function assertOpenApiSourcePath(appDir: string, sourcePath: string, configured: string): void {
  ...
  if (rel === '..' || rel.startsWith(`..${sep}`) || rel === '') {
    throw new DocsError(
      ...
      'docs_openapi_path_invalid',
      [{ code: 'openapi_path_escape', message: configured, file: sourcePath }],
    )
  }
}

export function operationMarkdown(operation: DocsOpenApiOperation): string {
  const lines = [
    `<div class="docs-api-badge"><span class="method method-${operation.method.toLowerCase()}">${operation.method}</span><code>${operation.path}</code></div>`,
    '',
  ]
...
): DocsOpenApiParameter[] {
  const merged = new Map<string, DocsOpenApiParameter>()
...
function invalidOpenApi(file: string, message: string): DocsError {
  return new DocsError(`Invalid OpenAPI document: ${message}`, 'docs_openapi_invalid', [
