...
const openApiExamplesSchema = z.object({
  languages: z.array(z.enum(DOCS_OPENAPI_SAMPLE_LANGUAGES)).min(1).optional(),
...
const contextualActionsSchema = z
  .array(z.enum(DOCS_CONTEXTUAL_ACTIONS))
  .min(1)
  .max(DOCS_CONTEXTUAL_ACTIONS.length)
...
export const rawConfigSchema = z.object({
  ...
  source: z.string().trim().min(1).default('docs'),
  output: z.string().trim().min(1).default('dist/_docs'),
  ...
  contextual: z
    .object({
      actions: contextualActionsSchema.default([...DEFAULT_DOCS_CONTEXTUAL_ACTIONS]),
    })
    .default({ actions: [...DEFAULT_DOCS_CONTEXTUAL_ACTIONS] }),
  seo: z
    .object({
