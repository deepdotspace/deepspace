...
export function loadDocsConfig(appDir: string): LoadedDocsConfig {
  const resolvedAppDir = resolve(appDir)
  const configPath = resolve(resolvedAppDir, DOCS_CONFIG_FILE)
  if (!existsSync(configPath)) {
    throw new DocsError(
      `No ${DOCS_CONFIG_FILE} found in ${resolvedAppDir}`,
      'docs_config_missing',
      [{ code: 'docs_config_missing', message: `Create ${DOCS_CONFIG_FILE}`, file: configPath }],
    )
  }
  ...
  try {
    ...
    throw new DocsError(
      `${DOCS_CONFIG_FILE} is not valid JSON: ${errorMessage(error)}`,
      'docs_config_invalid',
  ...
  if (!parsed.success) {
    ...
    throw new DocsError(
      `${DOCS_CONFIG_FILE} has ${diagnostics.length} validation error(s)`,
      'docs_config_invalid',
      diagnostics,
    )
  }
  ...
  if (
    ...
    const supported = listDeepSpaceAgentModels('documentation')
      ...
      .join(', ')
    throw new DocsError(
      `${DOCS_CONFIG_FILE} has an unsupported assistant model`,
      'docs_config_invalid',
      [
        {
  ...
  const config: DocsConfig = {
    ...
    navigation: navigation as DocsNavigationItem[] | undefined,
    links: data.links.length > 0 ? data.links : navbarLinks,
    footer,
    redirects: data.redirects,
    openapi: normalizeOpenApi(data.openapi, data.api) as DocsOpenApiConfig[],
  ...
  if (sourceDir === outputDir || isWithin(sourceDir, outputDir)) {
    throw new DocsError(
      'docs.json output must not contain the documentation source directory',
      'docs_path_collision',
      [
        {
          code: 'docs_path_collision',
    ...
    return {
      ...
      examples: {
        languages: localExamples?.languages ??
          api?.examples?.languages ?? [...DEFAULT_DOCS_OPENAPI_EXAMPLES.languages],
        defaults: localExamples?.defaults ??
          api?.examples?.defaults ?? DEFAULT_DOCS_OPENAPI_EXAMPLES.defaults,
        autogenerate: localExamples?.autogenerate ??
          api?.examples?.autogenerate ?? DEFAULT_DOCS_OPENAPI_EXAMPLES.autogenerate,
      },
    }
  })
}
